...
#define _SI_ASSERT(expr, exprStr, filename, line, ...) if(!expr){ SI::AssertInternal(exprStr, filename, line, __VA_ARGS__); }
...
#define _SI_WARNING(expr, exprStr, filename, line, ...) if(!expr){ SI::WarningInternal(exprStr, filename, line, __VA_ARGS__); }
