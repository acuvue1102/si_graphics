...
public:
    ...
    operator const D3D12_STATE_OBJECT_DESC&()
    {
        ...
        {
			if(Iter->Type == D3D12_STATE_SUBOBJECT_TYPE_SUBOBJECT_TO_EXPORTS_ASSOCIATION)
			{
				auto* ppp = &(*Iter);
				printf("ASSOS = %p\n", ppp);
			}
            ...
            Iter->pSubobjectArrayLocation = &(*Iter); // Store new location in array so we can redirect pointers contained in subobjects 
        }
